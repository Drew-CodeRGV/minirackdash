...
class EeroAPI:
        ...
        try:
            if os.path.exists(API_TOKEN_FILE):
                with open(API_TOKEN_FILE,'r')as f:
                    ...
                    return token
        except Exception as e:logging.error(f"Error loading token: {{e}}")
        return None
    ...
    def get_all_devices(self):
        try:
            ...
            logging.debug(f"Response structure: {{list(d.keys())}}")
            if'data'in d:
                if isinstance(d['data'],list):return d['data']
                elif isinstance(d['data'],dict)and'devices'in d['data']:return d['data']['devices']
            logging.warning(f"Unexpected response structure")
            return[]
        except Exception as e:logging.error(f"Device fetch error: {{e}}");import traceback;logging.error(traceback.format_exc());return[]
...
def update_cache():
    ...
    try:
        ...
        logging.info(f"Processing {{len(all_devs)}} total devices")
        ...
        for dev in wireless:
            ...
            if sig_dbm is not None:
                try:sigs.append(float(sig_dbm)if isinstance(sig_dbm,(int,float))else float(str(sig_dbm).replace(' dBm','').strip()));logging.debug(f"Added signal: {{sig_dbm}} dBm")
                except:pass
        ...
        logging.info(f"Device OS: {{dos}}")
        logging.info(f"Frequency: {{fd}}")
        logging.info(f"Processed {{len(devs)}} devices")
        logging.info("Cache update complete")
        logging.info("="*60)
    except Exception as e:logging.error(f"Cache error: {{e}}");import traceback;logging.error(traceback.format_exc())
def run_speedtest():
    global data_cache
    try:
...
def version():
    c=load_config()
    return jsonify({{'version':'3.0.5','name':'Eero Dashboard','network_id':c.get('network_id',eero_api.network_id)}})
@app.route('/api/admin/check-update')
def check_update():
    try:
        with urllib.request.urlopen(SCRIPT_URL_V3,timeout=10)as r:ls=r.read().decode('utf-8')
        lv=extract_ver(ls)
        return jsonify({{'current_version':'3.0.5','latest_version':lv or'3.0.5','update_available':compare_ver(lv or'3.0.5','3.0.5')>0}})
    except:return jsonify({{'current_version':'3.0.5','latest_version':'3.0.5','update_available':False}})
@app.route('/api/admin/update',methods=['POST'])
def update_sys():
    try:
        with urllib.request.urlopen(SCRIPT_URL_V3,timeout=10)as r:ls=r.read().decode('utf-8')
        lv=extract_ver(ls)
        if not lv or compare_ver(lv,'3.0.5')<=0:return jsonify({{'success':False,'message':'Already latest'}})
...
if __name__=='__main__':
    logging.info("Starting Eero Dashboard Backend v3.0.5")
    update_cache()
...
def main():
    ...
    try:
        ...
        print()
        pc(C.Y,"⚠️  IMPORTANT: Authenticate API Access")
        pc(C.C,"Run: sudo python3 /home/eero/dashboard/setup_eero_auth.py")
        print()
        pi("Dashboard: http://localhost")
        pi("Admin: Click π icon (bottom-right)")
        pi("Logs: tail -f /home/eero/dashboard/logs/backend.log")
