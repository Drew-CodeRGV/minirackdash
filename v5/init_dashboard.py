    ...
    try:
        ...
        if result.stdout.strip():
            ...
            if response in ['yes', 'y']:
                ...
                print_success("Stopped web services")
                return True
            else:
                print_error("Port 80 must be available. Installation cannot continue.")
                return False
        else:
...
def force_update_from_cloud():
    """Force download and update script from GitHub"""
    try:
        ...
        print_error(f"Force update failed: {e}")
...
def check_for_updates():
    ...
    if response and response.lower() in ['yes', 'y']:
        print_info("Forcing update from GitHub...")
        force_update_from_cloud()
    else:
        print_info("Proceeding with current version")
...
def install_dependencies():
    ...
    run_command("DEBIAN_FRONTEND=noninteractive apt-get install -y python3 python3-pip python3-venv git curl speedtest-cli unclutter x11-xserver-utils xdotool lsof", timeout=600, show=True)
    ...
    try:
        ...
        for device in wireless:
            ...
            if freq_band in freq_dist:
                freq_dist[freq_band] += 1
            
            sig_dbm = conn.get('signal_avg')
            score_bars = conn.get('score_bars', 0)
            
            if sig_dbm is None and score_bars:
                sig_dbm = estimate_signal_from_bars(score_bars)
            
            sig_pct = convert_signal_dbm_to_percent(sig_dbm)
            
            if sig_dbm is not None:
                try:
                    signals.append(float(sig_dbm) if isinstance(sig_dbm, (int, float)) else float(str(sig_dbm).replace(' dBm', '').strip()))
                except:
                    pass
...
if __name__ == '__main__':
    ...
    logging.info("Port 80 is available")
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { background: linear-gradient(135deg, #001a33 0%, #003366 100%); font-family: 'Segoe UI', sans-serif; color: #fff; overflow: hidden; height: 100vh; }
        .header { background: rgba(0,20,40,.9); padding: 8px 20px; display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid rgba(77,166,255,.3); }
        .header-title { font-size: 18px; font-weight: 600; color: #4da6ff; }
        .header-actions { display: flex; gap: 10px; align-items: center; }
        .header-btn { padding: 6px 12px; background: rgba(77,166,255,.2); border: 2px solid #4da6ff; border-radius: 6px; color: #fff; cursor: pointer; display: flex; align-items: center; gap: 6px; font-size: 12px; transition: all .3s; }
        .header-btn:hover { background: rgba(77,166,255,.4); transform: translateY(-2px); }
        .status-indicator { display: flex; align-items: center; gap: 6px; padding: 6px 12px; background: rgba(0,0,0,.3); border-radius: 15px; font-size: 11px; }
