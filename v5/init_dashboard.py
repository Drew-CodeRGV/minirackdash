...
def authenticate_eero(network_id):
    ...
    try:
        ...
        response = requests.post("https://api-user.stage.e2ro.com/2.2/pro/login", json=login_payload, timeout=10)
        ...
        verify_url = "https://api-user.stage.e2ro.com/2.2/login/verify"
...
EERO_API_BASE="https://api-user.stage.e2ro.com/2.2"
...
def get_version():
    config=load_config()
    return jsonify({{'version':CURRENT_VERSION,'name':'Eero Dashboard','network_id':config.get('network_id',eero_api.network_id)}})
...
def reauthorize():
    try:
        ...
        if step=='send':
            ...
            r=requests.post("https://api-user.stage.e2ro.com/2.2/pro/login",json={{"login":email}},timeout=10)
            r.raise_for_status()
            rd=r.json()
            ...
            vr=requests.post("https://api-user.stage.e2ro.com/2.2/login/verify",headers={{"X-User-Token":token}},data={{"code":code}},timeout=10)
            vr.raise_for_status()
            vd=vr.json()
...
def create_service():
    ...
    svc=f"""[Unit]
Description=Eero Dashboard v5.1
...
ExecStart={INSTALL_DIR}/venv/bin/python3 eero_api.py
Restart=always
RestartSec=10
[Install]
...
def main():
    os.system('clear')
    print_header(f"Eero Dashboard v5.1 Installer - v{SCRIPT_VERSION}")
