...
def prompt_environment():
    ...
    config = load_config()
    saved_env = config.get('environment')
    
    if saved_env:
        env_name = "Production" if saved_env == "production" else "Staging"
        print_info(f"Saved Environment: {env_name}")
        print_color(Colors.MAGENTA, "Press Enter to use saved environment, or type 'change' to select new:")
        print_color(Colors.CYAN, "Choice: ")
        choice = input().strip().lower()
        if choice != 'change':
            print_warning(f"Using saved environment: {env_name}")
            return saved_env
    
    print_info("Is this network on Production?")
    print_color(Colors.CYAN, "  [Y] Yes - Production (api-user.e2ro.com)")
    print_color(Colors.CYAN, "  [N] No  - Staging (api-user.stage.e2ro.com)")
    print()
    print_color(Colors.MAGENTA, "Enter choice (Y/N): ")
    ...
    print_success(f"Environment set to: {env_name} ({api_url})")
    return environment
...
def authenticate_eero(network_id, api_url):
...
def install_dependencies():
    ...
    run_command("DEBIAN_FRONTEND=noninteractive apt-get install -y python3 python3-pip python3-venv git curl speedtest-cli unclutter x11-xserver-utils xdotool", timeout=600, show=True)
...
def setup_python():
    ...
    run_command(f'sudo -u {USER} {INSTALL_DIR}/venv/bin/pip install --quiet flask flask-cors requests speedtest-cli', timeout=300)
...
def get_api_url():
    config=load_config()
    return config.get('api_url','api-user.stage.e2ro.com')
class EeroAPI:
    def __init__(self):
    ...
    def get_headers(self):
        headers={{'Content-Type':'application/json','User-Agent':'Eero-Dashboard/5.1.1'}}
...
def get_version():
    ...
    return jsonify({{'version':CURRENT_VERSION,'name':'Eero Dashboard','network_id':config.get('network_id',eero_api.network_id),'environment':env,'api_url':config.get('api_url','api-user.stage.e2ro.com')}})
@app.route('/api/admin/check-update')
def check_update():
    try:
        with urllib.request.urlopen(SCRIPT_URL_V5,timeout=10)as r:latest_script=r.read().decode('utf-8')
        latest_version=extract_version_from_script(latest_script)
        return jsonify({{'current_version':CURRENT_VERSION,'latest_version':latest_version or CURRENT_VERSION,'update_available':compare_versions(latest_version or CURRENT_VERSION,CURRENT_VERSION)>0}})
    except:return jsonify({{'current_version':CURRENT_VERSION,'latest_version':CURRENT_VERSION,'update_available':False}})
...
if __name__=='__main__':
    logging.info("Starting Eero Dashboard Backend v5.1.1")
...
def create_service():
    ...
    svc=f"""[Unit]
Description=Eero Dashboard v5.1.1
...
ExecStart={INSTALL_DIR}/venv/bin/python3 {INSTALL_DIR}/backend/eero_api.py
Restart=always
RestartSec=10
...
"""
    with open(f'{autostart_dir}/dashboard.desktop','w')as f:f.write(desktop)
...
def main():
    os.system('clear')
    print_header(f"Eero Dashboard v5.1.1 Installer - v{SCRIPT_VERSION}")
