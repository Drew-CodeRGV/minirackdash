...
if [ "$CSS_ISSUE" = true ]; then
    ...
    echo "📥 Downloading fixed files (v6.7.4-mobile)..."
    ...
    if grep -q "6.7.4" /opt/eero/app/index.html && grep -q "6.7.4" /opt/eero/app/dashboard.py; then
    ...
    for i in {1..10}; do
        if curl -s http://localhost:5000/ 2>/dev/null | grep -q "6.7.4"; then
            echo "✅ Dashboard is responding with v6.7.4"
            break
        else
