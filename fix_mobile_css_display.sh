...
if curl -s http://localhost:5000/ | grep -q "6.7.4" 2>/dev/null; then
    echo "✅ CSS fix applied successfully - v6.7.4-mobile"
    ...
    echo ""
    echo "🎉 Mobile CSS display issue fixed!"
    echo "🌐 Dashboard: http://$PUBLIC_IP"
    echo ""
    echo "✅ CSS should now render properly (no text display)"
    echo "✅ Mobile responsive design active"
    echo "📲 Test on mobile device to verify fix"
else
