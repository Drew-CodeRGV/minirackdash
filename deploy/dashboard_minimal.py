    ...
    def get_network_info(self):
        ...
        try:
            ...
            if 'data' in data:
                ...
                logging.info("Retrieved network info: " + network_data.get('name', 'Unknown'))
                return network_data
            return {}
        except Exception as e:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if 'data' in data:
                    ...
                    logging.info("Retrieved " + str(len(devices)) + " devices (attempt " + str(attempt + 1) + ")")
                    return devices
                
                logging.warning("No data in response (attempt " + str(attempt + 1) + "): " + str(data))
                return []
                
            except requests.exceptions.Timeout:
                logging.warning("API timeout on attempt " + str(attempt + 1))
                if attempt < max_retries - 1:
                    time.sleep(2 ** attempt)
                    continue
            except requests.exceptions.RequestException as e:
                logging.warning("API request error on attempt " + str(attempt + 1) + ": " + str(e))
                if attempt < max_retries - 1:
                    time.sleep(2 ** attempt)
                    continue
            except Exception as e:
                logging.error("Device fetch error on attempt " + str(attempt + 1) + ": " + str(e))
                if attempt < max_retries - 1:
                    time.sleep(2 ** attempt)
                    continue
...
def update_cache():
    ...
    try:
        ...
        if len(connected_devices) == 0 and previous_device_count > 0:
            logging.warning("API returned 0 devices but we had " + str(previous_device_count) + " before - keeping previous cache")
            return
        
        wireless_devices = [d for d in connected_devices if d.get('wireless')]
        
        logging.info("Processing " + str(len(connected_devices)) + " connected devices (" + str(len(wireless_devices)) + " wireless)")
        ...
        logging.info("Cache updated successfully: " + str(len(connected_devices)) + " total devices (" + str(len(wireless_devices)) + " wireless, " + str(len(connected_devices) - len(wireless_devices)) + " wired)")
